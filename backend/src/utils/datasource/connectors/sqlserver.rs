...
type ConnectionParseResult = Result<(String, u16, Option<String>, String, String), Box<dyn Error + Send + Sync>>;
...
impl SqlServerConnector {
    ...
    fn parse_connection_url(url: &str) -> ConnectionParseResult {
        ...
        let url = if let Some(stripped) = url.strip_prefix("sqlserver://") {
            stripped
        } else if let Some(stripped) = url.strip_prefix("mssql://") {
            stripped
        } else {
            return Err(
                "Invalid SQL Server URL format. Must start with sqlserver:// or mssql://".into(),
            );
        };
    ...
    async fn get_connection(&self) -> Result<Client<Compat<TcpStream>>, Box<dyn Error + Send + Sync>> {
    ...
    async fn create_new_connection(&self) -> Result<Client<Compat<TcpStream>>, Box<dyn Error + Send + Sync>> {
        // Connect with 3-second timeout
        let tcp = tokio::time::timeout(Duration::from_secs(3), TcpStream::connect(&self.server))
            .await
            .map_err(|_| Box::<dyn Error + Send + Sync>::from("Connection timeout after 3 seconds"))??;
    ...
    async fn test_connection(&mut self) -> Result<bool, Box<dyn Error + Send + Sync>> {
        ...
        for (config, ssl_mode) in attempts {
            ...
            info!(
                ...
                attempt_num, ssl_status
            );

            // Try to connect with 3-second timeout
            match tokio::time::timeout(Duration::from_secs(3), TcpStream::connect(&self.server))
                .await
            {
                Ok(Ok(tcp)) => {
                                    ...
                                    match stream.into_results().await {
                                        Ok(_) => {
                                            ...
                                            if ssl_mode == Some("disable".to_string()) {
                                                info!(
                                                    "Saved working configuration with SSL disabled"
                                                );
                                            } else {
                                                info!(
                                                    "Saved working configuration with SSL enabled"
                                                );
                                            }
                                        ...
                                        Err(e) => {
                                            last_error =
                                                Some(Box::new(e) as Box<dyn Error + Send + Sync>);
                                        }
                                    }
                                }
                                Err(e) => {
                ...
                Err(_) => {
                    last_error = Some(Box::<dyn Error + Send + Sync>::from(
        ...
        if let Some(e) = last_error {
            ...
            if error_string.contains("Login failed") {
                ...
                || error_string.contains("No connection")
            {
            ...
            } else if error_string.contains("certificate")
    ...
    async fn execute_query(&self, query: &str, limit: i32) -> Result<Value, Box<dyn Error + Send + Sync>> {
        ...
        if self.schema != "dbo" {
            ...
            let schema_context = format!(
                "EXEC('USE [{}]')",
                self.database.as_ref().unwrap_or(&"master".to_string())
            );
            match client.execute(&schema_context, &[]).await {
                Ok(_) => debug!("Set database context for schema operations"),
                Err(e) => debug!(
                    "Could not set database context: {}. Tables may need to be fully qualified.",
                    e
                ),
            }
        }
        ...
        let query_with_limit =
            if query.to_lowercase().contains("top ") || query.to_lowercase().contains("limit ") {
                query.to_string()
            } else {
