...
pub fn auth_routes() -> Router {
    Router::new()
        ...
        .push(Router::with_path("/auth/me").get(me))
        .push(Router::with_path("/auth/change-password").post(change_password))
        .push(Router::with_path("/auth/registration-status").get(check_registration_status))
        .push(Router::with_path("/auth/clients").get(list_public_clients))
        .push(Router::with_path("/auth/clients/all").get(list_all_clients))
        .push(Router::with_path("/auth/clients/detect-by-domain").get(detect_client_by_domain))
        .push(Router::with_path("/auth/users/exists").get(check_users_exist))
}

pub fn auth_protected_routes() -> Router {
    Router::new().push(Router::with_path("/auth/session-token").get(get_session_token))
}
