...
use crate::models::client::{
    ClientAdminResponse, ClientCreateRequest, ClientRootResponse, ClientStatus, ClientUpdateRequest,
};
...
pub async fn list_clients_admin(depot: &mut Depot, res: &mut Response) -> Result<(), AppError> {
    ...
    let user_client_id = depot.get::<String>("current_user_client_id").ok(); // Convert Result to Option

    // Build query based on role
    let query = if role.as_str() == "admin" {
        // Admin can only see their own client
        let client_id_str = user_client_id.ok_or_else(|| {
            AppError::InternalServerError("Client ID not found for admin".to_string())
        })?;
        let client_id = Uuid::parse_str(client_id_str.as_str())
            .map_err(|_| AppError::InternalServerError("Invalid client ID".to_string()))?;
    ...
    let clients: Vec<ClientAdminResponse> = rows
        .iter()
        .map(|row| {
    ...
    if role.as_str() == "admin" {
        ...
        if client_uuid != user_client_uuid {
            return Err(AppError::Forbidden(
                "You can only view your own client".to_string(),
            ));
        }
    }
...
pub async fn list_clients_root(depot: &mut Depot, res: &mut Response) -> Result<(), AppError> {
    ...
    let clients: Vec<ClientRootResponse> = rows
        .iter()
        .map(|row| {
            let status_str: String = row.get("status");
            let status = match status_str.as_str() {
...
pub async fn update_client(
    ...
    let update_req: ClientUpdateRequest = req.parse_json().await.map_err(|e| {
    ...
    if result.is_none() {
        return Err(AppError::NotFound(
            "Client not found or already deleted".to_string(),
        ));
    }
...
pub async fn delete_client(
    ...
    if user_count > 0 {
        return Err(AppError::BadRequest(format!(
            "Cannot delete client with {} active users",
            user_count
        )));
    }
    ...
    sqlx::query(
        "UPDATE clients SET deleted_at = $1, updated_at = $1 WHERE id = $2 AND deleted_at IS NULL",
    )
    .bind(Utc::now())
    .bind(client_uuid)
...
pub async fn disable_client(
    ...
    sqlx::query(
        "UPDATE clients SET status = 'error', updated_at = $1 WHERE id = $2 AND deleted_at IS NULL",
    )
    .bind(Utc::now())
    .bind(client_uuid)
    .execute(&state.db_pool)
...
pub async fn update_client_config(
    ...
    let config_req: UpdateConfigRequest = req
        .parse_json()
        .await
    ...
    sqlx::query(
        "UPDATE clients SET config = $1, updated_at = $2 WHERE id = $3 AND deleted_at IS NULL",
    )
...
#[handler]
pub async fn update_client_domains(
    req: &mut Request,
    depot: &mut Depot,
    res: &mut Response,
) -> Result<(), AppError> {
    ...
    let domains_req: UpdateDomainsRequest = req
        .parse_json()
        .await
        .map_err(|_| AppError::BadRequest("Invalid JSON".to_string()))?;
    ...
    sqlx::query(
        "UPDATE clients SET domains = $1, updated_at = $2 WHERE id = $3 AND deleted_at IS NULL",
    )
...
pub async fn suspend_client(
    ...
    sqlx::query(
        "UPDATE clients SET status = $1, updated_at = $2 WHERE id = $3 AND deleted_at IS NULL",
    )
    .bind("suspended")
    .bind(Utc::now())
    .bind(client_uuid)
...
#[handler]
pub async fn create_client_root(
    req: &mut Request,
    depot: &mut Depot,
    res: &mut Response,
) -> Result<(), AppError> {
...
pub async fn set_claude_token(
    ...
    let token_request: SetClaudeTokenRequest = req
        .parse_json()
        .await
        .map_err(|_| AppError::BadRequest("Invalid JSON".to_string()))?;
    ...
    let client_row = sqlx::query(
        "SELECT id, name, install_path FROM clients WHERE id = $1 AND deleted_at IS NULL",
    )
    .bind(client_uuid)
    .fetch_optional(&state.db_pool)
    .await
    ...
    match ClaudeManager::submit_token(client_uuid, &token_request.claude_token).await {
        Ok(oauth_token) => {
            ...
            tracing::info!(
                "Claude token set successfully for client {} by root user",
                client_uuid
            );
