...
#[handler]
pub async fn create_user_admin(req: &mut Request, depot: &mut Depot, res: &mut Response) -> Result<(), AppError> {
    let create_req: CreateUserRequest = req.parse_json().await
        .map_err(|_| AppError::BadRequest("Invalid JSON".to_string()))?;
    ...
    let user_role = depot.get::<String>("current_user_role")
    ...
    let user_response = UserResponse {
        ...
        client_id,
        username: create_req.username,
        role: if role_str == "user" { UserRole::User } else { role },
        status: UserStatus::Active,
        last_active: None,
        created_at: now,
        updated_at: now,
...
#[handler]
pub async fn update_user_admin(req: &mut Request, depot: &mut Depot, res: &mut Response) -> Result<(), AppError> {
    let user_id = req.param::<String>("user_id")
        .ok_or_else(|| AppError::BadRequest("Missing user ID".to_string()))?;
    ...
    let current_user_id = depot.get::<String>("current_user_id")
        .map_err(|_| AppError::InternalServerError("Failed to get user ID from depot".to_string()))?;
    ...
    if target_client_id != client_id {
        return Err(AppError::Forbidden("Cannot update users from other clients".to_string()));
    }
    
    // Admins cannot modify other admins or root users
    if user_role == "admin" && (target_role == "admin" || target_role == "root") && user_uuid != current_user_uuid {
        return Err(AppError::Forbidden("Cannot modify other administrators".to_string()));
    }
    ...
    if let Some(ref role) = update_req.role {
        ...
        if user_role == "admin" && *role != UserRole::User {
            return Err(AppError::Forbidden("Cannot promote users to admin role".to_string()));
        }
        param_count += 1;
        updates.push(format!("role = ${}", param_count));
    }
    ...
    if let Some(role) = update_req.role {
        let role_str = match role {
            UserRole::User => "user",
            UserRole::Admin => if user_role == "admin" { "user" } else { "admin" },
            UserRole::Root => "root",
        };
        query = query.bind(role_str);
    }
...
#[handler]
pub async fn delete_user_admin(req: &mut Request, depot: &mut Depot, res: &mut Response) -> Result<(), AppError> {
    let user_id = req.param::<String>("user_id")
        .ok_or_else(|| AppError::BadRequest("Missing user ID".to_string()))?;
    ...
    if user_client_id != client_id {
        return Err(AppError::Forbidden("Cannot delete users from other clients".to_string()));
    }
    
    // Admins cannot delete other admins or root users
    if user_role == "admin" && (user_role_str == "admin" || user_role_str == "root") {
        return Err(AppError::Forbidden("Cannot delete administrator accounts".to_string()));
    }
...
pub fn admin_routes() -> Router {
    Router::new()
        ...
        .push(Router::with_path("users/{user_id}")
