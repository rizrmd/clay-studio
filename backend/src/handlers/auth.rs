...
pub async fn me(depot: &mut Depot, res: &mut Response) -> Result<(), AppError> {
    ...
    let row = row.ok_or_else(|| AppError::Unauthorized("User not found".to_string()))?;
    ...
    // Check if initial setup is complete by checking if any clients exist
    let clients_exist = sqlx::query("SELECT COUNT(*) as count FROM clients")
        .fetch_one(&state.db_pool)
        .await
        .map_err(|e| AppError::InternalServerError(format!("Database error: {}", e)))?;
    
    let client_count: i64 = clients_exist.get("count");
    let is_setup_complete = client_count > 0;
