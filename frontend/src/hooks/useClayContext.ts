...
// Types matching the backend context service
...
export function useConversationContext(conversationId: string | null) {
  ...
  return {
    ...
    activeTool: context?.available_tools.filter((t) => t.applicable) || [],
...
export function useProjectContext(projectId: string | null) {
  const {
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `${API_BASE_URL}/chat/projects/${projectId}/context`
      );
...
export function useConversationSummary(conversationId: string | null) {
  ...
  const { data: summary, isLoading } = useQuery<ConversationSummary | null>({
    queryKey: ["conversationSummary", conversationId],
    queryFn: async () => {
      if (!conversationId) return null;

      const response = await fetch(
        `${API_BASE_URL}/chat/conversations/${conversationId}/summary`
      );
  ...
  return {
    summary,
    isLoading,
    createSummary: (summaryText: string) =>
      conversationId
        ? createSummaryMutation.mutate({ conversationId, summaryText })
...
/**
 * Enhanced chat hook with smart context loading
 */
export function useClayChat(projectId: string, conversationId?: string) {
  ...
  // Initialize messages from context when available
  useEffect(() => {
    if (conversationContext?.messages) {
      setMessages(conversationContext.messages);
    }
  }, [conversationContext]);
    ...
    async (content: string) => {
      ...
      try {
        ...
        const assistantMessage: ContextMessage = {
          ...
          created_at: assistantResponse.createdAt,
        };
