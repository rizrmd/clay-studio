...
export const checkAuthStatus = async () => {
  try {
    ...
    // Add client name from firstClient if available
    if (userData && authStore.firstClient?.name) {
      userData.clientName = authStore.firstClient.name;
    }
...
export const detectAndSetClientByDomain = async () => {
  try {
    ...
    const url = `/auth/clients/detect-by-domain?domain=${encodeURIComponent(
      currentDomain
    )}`;
    ...
    if (response.data.found && response.data.client) {
      ...
      const clientData: Client = {
        ...
        status: response.data.client.status as
          | "pending"
          | "installing"
          | "active"
          | "error",
    ...
    console.error(
      "[Domain Detection] Failed to detect client by domain:",
      error
    );
    return null;
  }
...
export const login = async (username: string, password: string) => {
  ...
  try {
    ...
    const userData = response.data.user;
    // Add client name from firstClient
    if (userData && authStore.firstClient?.name) {
      userData.clientName = authStore.firstClient.name;
    }
